...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['atualizar'])) {
    ...
    $cores = !empty($_POST['cores']) ? json_encode(array_map('trim', explode(',', $_POST['cores']))) : null;

    $sql = "UPDATE produto 
            SET nome = :nome, descricao = :descricao, preco = :preco, estoque = :estoque, 
                categoria_id = :categoria_id, genero_id = :genero_id, cores = :cores
            WHERE id = :id";
...
img { max-width: 100px; }
.cor-box { width: 22px; height: 22px; border-radius: 5px; display: inline-block; margin: 3px; border: 1px solid #aaa; }
...
<label>Cores (hexadecimal, separados por vírgula):</label>
<input type="text" name="cores" value="<?= htmlspecialchars($produtoEditar['cores']) ?>">
